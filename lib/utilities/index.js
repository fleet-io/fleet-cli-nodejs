...
module.exports = {
    ...
    updateFleetStatus: function(state) {
        let fleetConfigData = {};
        if(this.isFleetConfigCreated()) {
            fleetConfigData = this.getFleetConfigFileData();
        }
    
        fleetConfigData.status = {state: state};
        if(arguments[1])
            fleetConfigData.status.eventData = arguments[1];

        this.updateFleetConfigFileData(fleetConfigData);
    },
