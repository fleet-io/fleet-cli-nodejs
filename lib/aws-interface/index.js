...
let handleCreateS3Bucket = function(fleetConfigData, websiteName, region, fleetPath, correlationData, data) {
    ...
    let url = `http://${bucketName}.s3-website.${region}.amazonaws.com`;
...
let createS3Bucket = async function(fleetConfigData, websiteName, region, fleetPath, correlationData) {
    let params = {
        Bucket: `fleet-${fleetConfigData.appName}-${websiteName}-${region}`
    }
...
let emptyS3Bucket  = async function(fleetConfigData, websiteName, region, fleetPath, correlationData) {
    let params = {
        Bucket: `fleet-${fleetConfigData.appName}-${websiteName}-${region}`
    }
    ...
    if(objectList.isTruncated) await emptyS3Bucket(fleetConfigData, websiteName, region, fleetPath, correlationData);
}

let deleteS3Bucket = async function(fleetConfigData, websiteName, region, fleetPath, correlationData) {
    let params = {
        Bucket: `fleet-${fleetConfigData.appName}-${websiteName}-${region}`
    }

    await s3.deleteBucket(fleetConfigData, region, params, fleetPath, correlationData).catch(e => {throw e});
}
...
let putS3WebsiteContent = async function(fleetConfigData, websiteName, region, fleetPath, correlationData) {
    ...
    for(let file in files) {
        ...
        var body = fs.readFileSync(files[file]);
        ...
        await s3.putObject(fleetConfigData, region, params, fleetPath, correlationData).catch(e => {throw e});
    }
}
...
module.exports = {
    ...
    ensureWebsiteUndeployed: async function(fleetConfigData, websiteName, region, fleetPath, correlationData) {
        if (fleetConfigData.websites[websiteName].url[region]) {
            ...
            await emptyS3Bucket(fleetConfigData, websiteName, region, fleetPath, correlationData).catch(e => {throw e});
            await deleteS3Bucket(fleetConfigData, websiteName, region, fleetPath, correlationData).catch(e => {throw e});
        }
    },
