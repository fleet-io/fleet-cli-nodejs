...
let catchPromiseError = (error) => {
    util.showLog(`${error} ${error.stack}`); // an error occurred
}
...
module.exports = {
    ...
    createLambdaFunction: async function(fleetConfigData, routeName, region) {
        if (fleetConfigData.routes[routeName].lambda &&
            ...
            await this.updateLambdaFunction(fleetConfigData, routeName, region);
        } else {
    ...
    describeRegions: async function(fleetConfigData) {
        let regions = [];
