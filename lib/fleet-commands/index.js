...
let createFleet = async function(appName, region, global, correlationData) {
    ...
    if(global) {
        ...
        region = await awsInterface.describeRegions(fleetConfigData, correlationData).catch(e => util.showLog(e, correlationData));
    }
    ...
    for(region in fleetConfigData.region) {
        await awsInterface.createAPIG(fleetConfigData, fleetConfigData.region[region], correlationData).catch(e => util.showLog(e, correlationData));
    }

    await awsInterface.createLambdaRole(fleetConfigData, correlationData).catch(e => util.showLog(e, correlationData));
    await awsInterface.createLambdaRoleInlinePolicy(fleetConfigData, correlationData).catch(e => util.showLog(e, correlationData));
...
exports.createFleet = async function() {
    if (util.isFleetConfigCreated()) {
        util.showLog(`fleet.json configuration file already exists.`, correlationData);
    } else {
        util.updateFleetStatus(`creating-fleet`);
        await createFleet.apply(null, arguments).catch(e => util.showLog(e, correlationData));
    }
...
exports.deleteFleet = async function() {
    ...
    await deleteFleet.apply(null, arguments).catch(e => util.showLog(e, correlationData));
}

exports.newRoute = async function() {
    util.updateFleetStatus(`new-route`, {route: arguments[0]});
    await newRoute.apply(null, arguments).catch(e => util.showLog(e, correlationData));
    util.updateFleetStatus(`ready`);
}

exports.runRoute = async function() {
    util.updateFleetStatus(`run-route`, {route: arguments[0]});
    await runRoute.apply(null, arguments).catch(e => util.showLog(e, correlationData));
    util.updateFleetStatus(`ready`);
}

exports.deployRoute = async function() {
    util.updateFleetStatus(`deploying-route`, {route: arguments[0]});
    await deployRoute.apply(null, arguments).catch(e => util.showLog(e, correlationData));
    util.updateFleetStatus(`ready`);
}

exports.undeployRoute = async function() {
    util.updateFleetStatus(`undeploying-route`, {route: arguments[0]});
    await undeployRoute.apply(null, arguments).catch(e => util.showLog(e, correlationData));
    util.updateFleetStatus(`ready`);
}

exports.newWebsite = async function() {
    util.updateFleetStatus(`new-website`, {route: arguments[0]});
    await newWebsite.apply(null, arguments).catch(e => util.showLog(e, correlationData));
    util.updateFleetStatus(`ready`);
}

exports.deployWebsite = async function() {
    util.updateFleetStatus(`deploy-website`, {route: arguments[0]});
    await deployWebsite.apply(null, arguments).catch(e => util.showLog(e, correlationData));
    util.updateFleetStatus(`ready`);
}
